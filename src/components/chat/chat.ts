...
import { Anchor, InputField } from "../index";
...
import { ActiveChat, ChatUser } from "../../type";
...
export class Chat extends Block<IProps, TRef> {
  constructor(props: IProps) {
    super({
      ...
      companion: (() => {
        const companion = props.activeChat?.users?.find(
          (user) => user.id !== window.store.getState().user?.id,
        );
        if (!companion) return false;
        const result = {
          ...companion,
          avatar: `${BASE_URL}/resources${companion.avatar}`,
        };
        return result;
      })(),
      ...
      addUserToChat: async (event) => {
        ...
        try {
          ...
          const users = await getChatParticipants(
            props.activeChat?.id as number,
          );
...
export default connect(({ isOpenDialogChatOptions, activeChat }) => ({
  isOpenDialogChatOptions,
  activeChat,
}))(Chat);
